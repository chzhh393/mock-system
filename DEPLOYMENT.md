...
| 环境 | 外网机器 | 内网机器 | 部署脚本 | 适用场景 |
|------|----------|----------|----------|----------|
| **环境 A/B** | 机器A: 192.168.123.66 | 机器B: 192.168.123.81 | `deploy-registry.sh` | 原始环境，支持 SSH |
| **环境 C/D** | 机器C: 192.168.123.113 | 机器D: 192.168.123.114 | `deploy-cd.sh` | 沙箱环境，无 SSH/Maven |

**如何选择环境**：
- 查看任务文档（如 `Task-*.md`）中指定的 IP 地址或机器名
- 如果任务提到 **机器A/B** 或 IP **192.168.123.66/81**，使用环境 A/B
- 如果任务提到 **机器C/D** 或 IP **192.168.123.113/114**，使用环境 C/D
...
| 机器 | 角色 | IP 地址 | 说明 |
|------|------|---------|------|
| 机器 C | 外网 | 192.168.123.113 | 部署 scp0005, outer-consumer, Registry |
| 机器 D | 内网 | 192.168.123.114 | 部署 scp0006, target-service |
...
适用于沙箱环境，**无需 SSH 和 Maven**，通过远程 Docker Daemon 部署。
...
**前置条件**：
- 机器 C/D 已启动 Docker Daemon TCP 监听 (端口 2375)
- 机器 C 已部署 Registry (端口 5000)
- 基础设施（MySQL, Kafka, Canal, Redis）已运行
